...
export const FlashDealSettingsView: React.FC = () => {
  ...
  const handleCreate = async () => {
    if (!selectedProductId || !newDeal.title) {
      alert('Fill all required fields');
      return;
    }
    await addFlashDeal({
      ...newDeal,
      targetIds: [selectedProductId],
    });
            ...
            <div key={deal.id} className="bg-white rounded-2xl border border-neutral-200 shadow-sm overflow-hidden group">
              <div className="p-4 border-b border-neutral-100 bg-neutral-50/50 flex items-center justify-between">
                <div className="flex items-center gap-3">
                  <div className={`w-2 h-2 rounded-full ${isActive ? 'bg-emerald-500 animate-pulse' : 'bg-neutral-300'}`} />
                  <span className="text-xs font-bold text-neutral-800">{deal.title}</span>
                </div>
                <div className="flex items-center gap-1.5">
                  <button 
                    onClick={() => updateFlashDeal(deal.id, { status: isActive ? 'paused' : 'active' })}
                  ...
                  <div className="flex gap-4">
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="flex items-center gap-2 mt-1">
                        ...
                        <span className="text-sm font-bold text-orange-600">₹{discountPrice}</span>
                ...
                <div className="grid grid-cols-2 gap-3 pt-2">
                  <div className="p-3 bg-neutral-50 rounded-xl border border-neutral-100">
                    ...
                    <div className="flex items-end justify-between mt-1">
                      <span className="text-lg font-bold text-neutral-800">{deal.lowStockThreshold || 5}</span>
