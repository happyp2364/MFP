        ...
        <div className="bg-[#121816] text-white p-3 sm:p-4 flex flex-col md:flex-row md:items-center justify-between gap-3 border-b border-white/10 shrink-0">
          ...
          <div className="flex items-center flex-wrap gap-2">
            ...
            <button
              ...
              className={`px-4 py-2 rounded-xl text-xs font-black flex items-center gap-2 shadow-xl transition-all border ${
                hasPendingDraft
                  ? 'bg-gradient-to-r from-emerald-500 via-[#0B8F63] to-teal-600 text-white border-emerald-300/40 hover:from-emerald-400 hover:to-teal-500 shadow-emerald-950/60 ring-2 ring-emerald-400/40 animate-pulse'
                  : 'bg-white/10 text-neutral-300 border-white/10 hover:bg-white/20'
              }`}
            >
              <span className="text-sm">🚀</span>
              <span className="tracking-wide uppercase font-black">Go Live</span>
              {hasPendingDraft && (
                <span className="bg-amber-400 text-neutral-950 font-black text-[10px] px-2 py-0.5 rounded-full shadow-sm ml-0.5">
                  Pending Changes: {pendingDraftCount}
                </span>
              )}
            </button>
                  ...
                  <div className="grid grid-cols-1 gap-4 text-xs">
                    ...
                    <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                      <div>
                        ...
                        <input
                          type="text"
                          value={typeof storeInfoForm.coordinates === 'string' ? storeInfoForm.coordinates : (storeInfoForm.coordinates ? `${storeInfoForm.coordinates.lat}, ${storeInfoForm.coordinates.lng}` : '')}
      ...
      {publishModalOpen && (
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
          ...
          <div className="relative w-full max-w-xl bg-white rounded-3xl shadow-2xl border border-neutral-100 p-6 space-y-5 z-10 animate-in zoom-in-95 duration-150 max-h-[90vh] overflow-y-auto">
            ...
            {/* INITIAL PRE-PUBLISH CONFIRMATION VIEW */}
            {!isPublishing && !publishResult && !publishError && (
              <div className="space-y-4">
                {/* Confirmation Box */}
                <div className="bg-gradient-to-r from-emerald-50 via-teal-50 to-emerald-50 border border-emerald-200/80 rounded-2xl p-4 text-xs text-emerald-950 space-y-2 shadow-sm">
                  <div className="flex items-center justify-between">
                    <div className="flex items-center gap-2 font-black text-emerald-950 text-sm">
                      <span className="text-base">🚀</span>
                      <span>Are you sure you want to make all pending changes live?</span>
                    </div>
                    <span className="bg-amber-400 text-neutral-950 font-black text-[10px] px-2.5 py-1 rounded-full shadow-sm shrink-0">
                      Pending Changes: {pendingChangesList.length}
                    </span>
                  </div>
                  <p className="text-[11px] text-emerald-800 leading-relaxed font-medium">
                    This single owner website system will atomically push all draft modifications live to all store visitors globally. Unchanged items will be preserved without duplicate writes.
                  </p>
                </div>

                {/* Changed Items List */}
                <div className="space-y-2">
                  <div className="flex items-center justify-between text-xs font-bold text-neutral-800 px-0.5">
                    <span>Pending Items to Publish ({pendingChangesList.length})</span>
                    <span className="text-[10px] font-semibold text-neutral-500">Draft → Live Diff</span>
                  </div>

                  <div className="max-h-48 overflow-y-auto space-y-1.5 p-2 bg-[#F9F9F9] rounded-2xl border border-neutral-200/80 text-xs">
                    {pendingChangesList.length === 0 ? (
                      <div className="p-4 text-center text-neutral-500 text-xs font-medium">
                        ✓ No pending changes detected. Live website is fully synchronized.
                      </div>
                    ) : (
                      pendingChangesList.map((item) => (
                        <div key={item.id} className="p-2.5 bg-white rounded-xl border border-neutral-200/60 shadow-2xs flex items-start justify-between gap-3">
                          <div className="space-y-0.5 min-w-0 flex-1">
                            <div className="font-bold text-neutral-900 truncate flex items-center gap-1.5">
                              {item.type === 'PRODUCT_ADD' && <span className="text-[9px] bg-emerald-100 text-emerald-800 font-extrabold px-1.5 py-0.5 rounded-md border border-emerald-300 shrink-0">NEW PRODUCT</span>}
                              {item.type === 'PRODUCT_UPDATE' && <span className="text-[9px] bg-sky-100 text-sky-800 font-extrabold px-1.5 py-0.5 rounded-md border border-sky-300 shrink-0">EDITED</span>}
                              {item.type === 'PRODUCT_DELETE' && <span className="text-[9px] bg-rose-100 text-rose-800 font-extrabold px-1.5 py-0.5 rounded-md border border-rose-300 shrink-0">REMOVED</span>}
                              {item.type === 'PRICE_CHANGE' && <span className="text-[9px] bg-purple-100 text-purple-800 font-extrabold px-1.5 py-0.5 rounded-md border border-purple-300 shrink-0">PRICE CHANGE</span>}
                              {item.type === 'STOCK_CHANGE' && <span className="text-[9px] bg-amber-100 text-amber-800 font-extrabold px-1.5 py-0.5 rounded-md border border-amber-300 shrink-0">STOCK UPDATE</span>}
                              {item.type === 'BANNER' && <span className="text-[9px] bg-indigo-100 text-indigo-800 font-extrabold px-1.5 py-0.5 rounded-md border border-indigo-300 shrink-0">HERO BANNER</span>}
                              {item.type === 'CATEGORIES' && <span className="text-[9px] bg-cyan-100 text-cyan-800 font-extrabold px-1.5 py-0.5 rounded-md border border-cyan-300 shrink-0">CATEGORIES</span>}
                              {item.type === 'OFFERS' && <span className="text-[9px] bg-rose-100 text-rose-800 font-extrabold px-1.5 py-0.5 rounded-md border border-rose-300 shrink-0">OFFERS</span>}
                              {item.type === 'SETTINGS' && <span className="text-[9px] bg-neutral-200 text-neutral-800 font-extrabold px-1.5 py-0.5 rounded-md border border-neutral-300 shrink-0">SETTINGS</span>}
                              {item.type === 'THEME' && <span className="text-[9px] bg-teal-100 text-teal-800 font-extrabold px-1.5 py-0.5 rounded-md border border-teal-300 shrink-0">THEME</span>}
                ...
                <div className="space-y-1">
                  <label className="block text-xs font-bold text-neutral-700">
                    Release Notes / Change Summary (Optional):
                  </label>
                  <textarea
                    rows={2}
                    placeholder="E.g., Updated footwear prices, adjusted stock levels, updated hero banner..."
                    value={publishSummary}
                    onChange={(e) => setPublishSummary(e.target.value)}
                    className="w-full bg-[#F7F7F7] border border-neutral-200 rounded-xl p-2.5 text-xs outline-none focus:ring-2 focus:ring-[#0B8F63] resize-none"
                  />
                </div>
                ...
                <div className="flex items-center justify-end gap-3 pt-2">
                  ...
                  <button
                    ...
                    className="bg-gradient-to-r from-emerald-500 via-[#0B8F63] to-teal-600 hover:from-emerald-400 hover:to-teal-500 text-white font-black text-xs px-6 py-2.5 rounded-xl shadow-lg shadow-[#0B8F63]/30 flex items-center gap-2 tracking-wide uppercase"
                  >
                    <span>🚀 Go Live</span>
                  </button>
                </div>
