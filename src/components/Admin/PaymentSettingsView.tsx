...
export const PaymentSettingsView: React.FC = () => {
  ...
  const [gatewayProvider, setGatewayProvider] = useState<'RAZORPAY' | 'PHONEPE' | 'CASHFREE' | 'PAYU' | 'DIRECT_UPI_QR'>(
    paymentSettings.gatewayProvider && paymentSettings.gatewayProvider !== 'DIRECT_UPI_QR'
      ? paymentSettings.gatewayProvider
      : 'RAZORPAY'
  );
  ...
  const handleTriggerQrPicker = (e?: React.MouseEvent | React.KeyboardEvent) => {
    if (e) {
      e.stopPropagation();
    }
    if (!isUploadingQr && fileInputRef.current) {
  ...
  const processQrFile = async (file: File) => {
    ...
    try {
      const optimized = await optimizeImageFile(file, { maxWidth: 1000, maxHeight: 1000, quality: 0.9 });
      setCustomQrImage(optimized);
      setQrMode('CUSTOM');
      setSaveStatus('IDLE');
      showToast('Custom QR image attached successfully!', 'success');
    } catch (err) {
      showToast('Failed to optimize QR image. Please try another file.', 'error');
    } finally {
  ...
  const handleQrDragOver = (e: React.DragEvent) => {
    e.preventDefault();
    e.stopPropagation();
    if (!isUploadingQr) setIsDraggingQr(true);
  };
  ...
  const executeSave = async () => {
    ...
    const updatedSettings: Partial<PaymentSettings> = {
      ...
      gatewayProvider: gatewayProvider === 'DIRECT_UPI_QR' ? 'RAZORPAY' : gatewayProvider,
      keyId: keyId.trim(),
      apiKey: keyId.trim(),
  ...
  const executeGatewayProbe = async () => {
    ...
    try {
      ...
      if (res.ok && data.success && data.connected) {
        setTestProbeResult({
          success: true,
          message: data.message || `✓ Gateway Connection Successful (${gatewayProvider} - ${data.mode || (isTestMode ? 'TEST' : 'LIVE')}).`,
        });
        showToast('Payment gateway connection test successful!', 'success');
      } else {
        setTestProbeResult({
          success: false,
          message: data.message || 'Gateway credentials were rejected. Please verify server environment variables (RAZORPAY_KEY_ID & RAZORPAY_KEY_SECRET).',
        });
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4 pt-1">
              ...
              <div className="p-3.5 bg-emerald-50/80 border border-emerald-200/80 rounded-xl space-y-1.5">
                <div className="flex items-center gap-2 text-emerald-950 font-bold text-xs">
                  ...
                  <span>Server-Side Secret Isolation (Strict Zero-Trust)</span>
                </div>
                <p className="text-[11px] text-emerald-900 leading-relaxed">
                  Your Razorpay Key Secret and Webhook Secret are managed exclusively through server environment variables (<code className="font-mono bg-white/70 px-1 py-0.5 rounded text-emerald-800">RAZORPAY_KEY_SECRET</code> & <code className="font-mono bg-white/70 px-1 py-0.5 rounded text-emerald-800">RAZORPAY_WEBHOOK_SECRET</code> in Vercel / Cloud Run). They are never stored in Firestore or exposed to client browsers.
                </p>
              </div>
            </div>
          ...
          <div className="p-5 bg-white rounded-2xl border border-neutral-200/80 shadow-sm space-y-4">
            ...
            {qrMode === 'CUSTOM' && (
              <div className="space-y-3 pt-1">
                <div
                  onClick={() => handleTriggerQrPicker()}
                  onDragOver={handleQrDragOver}
                  onDragEnter={handleQrDragOver}
                  onDragLeave={handleQrDragLeave}
                  onDrop={handleQrDrop}
                  role="button"
                  tabIndex={0}
                  aria-label="Click or drag custom QR image to upload"
                  onKeyDown={(e) => {
                    if (e.key === 'Enter' || e.key === ' ') {
                      e.preventDefault();
                      handleTriggerQrPicker();
                    }
                  }}
                  className={`border-2 border-dashed rounded-2xl p-4 text-center cursor-pointer transition-all select-none ${
                    isDraggingQr
                      ...
                      : 'border-neutral-300 hover:border-[#0B8F63] bg-neutral-50/50 hover:bg-emerald-50/20'
                  }`}
                >
                  {isUploadingQr ? (
                    <div className="py-6 flex flex-col items-center justify-center gap-2 text-emerald-800 text-xs font-bold">
                      <Loader2 className="w-7 h-7 text-[#0B8F63] animate-spin" />
                      <span>Optimizing and attaching custom QR code...</span>
                    </div>
                  ) : customQrImage ? (
                    <div className="space-y-3" onClick={(e) => e.stopPropagation()}>
                      ...
                      <div className="flex items-center justify-center gap-2">
                        <button
                          ...
                          className="px-3 py-1.5 bg-neutral-200 hover:bg-neutral-300 text-neutral-800 rounded-lg text-xs font-bold transition-colors cursor-pointer"
                        >
                          Change Image
                        </button>
                        <button
                          ...
                          onClick={(e) => {
                            e.stopPropagation();
                            handleRemoveCustomQr();
                          }}
                          className="px-3 py-1.5 bg-rose-100 hover:bg-rose-200 text-rose-700 rounded-lg text-xs font-bold flex items-center gap-1 transition-colors cursor-pointer"
                        >
                    ...
                    <div className="space-y-2 py-3">
                      <div className="w-10 h-10 rounded-full bg-emerald-100 text-[#0B8F63] flex items-center justify-center mx-auto pointer-events-none">
                        <Upload className="w-5 h-5" />
                      </div>
                      <p className="font-bold text-neutral-800 text-xs pointer-events-none">
                        Click or drag custom QR image to upload
                      </p>
                      <p className="text-[11px] text-neutral-500 pointer-events-none">
